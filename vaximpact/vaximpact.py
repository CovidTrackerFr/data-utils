...
def export_results_json(content, file_name, region_trigram):
    ...
    logger.info(f"[SUCCESS] - File {file_name} has been exported.")
...
class Vaximpact:
    ...
    def access_api(self):
        ...
        logger.info(f"[SUCCESS] - Successfully downloaded data from DREES {self.api_name} API.")
    ...
    def build_sum_by_week(self):
        ...
        logger.info(f"[SUCCESS] - Successfully built data table {self.api_name}.")
        return sum_by_week
    ...
    def calculate_stats(self):
        ...
        for week_number,week in self.week_table.items():
            ...
            for data_name, data_code in data_type.items():
                if week["Vaccinés"][f"{data_code}_pcr"] == 0:
                    risque_relatif=f"Non calculable - 0 {data_name} chez vaccinés"
                    if week["Pas_vaccinés"][f"{data_code}_pcr"] == 0:
                        ...
                        FER_exposes=100
                        FER_population=round((FER_exposes*(week["Pas_vaccinés"][f"{data_code}_pcr"]/(week["Pas_vaccinés"][f"{data_code}_pcr"]+week["Vaccinés"][f"{data_code}_pcr"]))),ROUND_DECIMAL)

                elif week["Pas_vaccinés"][f"{data_code}_pcr"] == 0:
                    ...
                    risque_relatif = round(((week["Pas_vaccinés"][f"{data_code}_pcr"]/week["Vaccinés"][f"{data_code}_pcr"])*(week["Vaccinés"]["effectif_j_7"]/week["Pas_vaccinés"]["effectif_j_7"])),ROUND_DECIMAL)
